    ...
    with TCPython(logging_policy=LoggingPolicy.NONE) as session:
        ...
        for idx, comp in rows:
            try:
                ...
                sol_k = res.get_value_of("Solidus temperature")
                liq_k = res.get_value_of("Liquidus temperature")
                results.append((idx, sol_k, liq_k))
            except Exception:
...
if __name__ == '__main__':
    ...
    with ProcessPoolExecutor(max_workers=max(1, args.max_workers)) as ex:
        futures = {ex.submit(_compute_family_tc, (active_el, rows)): active_el for active_el, rows in families.items()}
        done_families = 0
        done_rows = 0
        for fut in as_completed(futures):
            ...
            results = {idx: (np.nan, np.nan) for idx, _ in rows}
            try:
                for idx, sol_k, liq_k in fut.result():
                    results[idx] = (sol_k, liq_k)
            except Exception:
                pass
