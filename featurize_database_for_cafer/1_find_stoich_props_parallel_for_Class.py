...
# --- Featurization helpers for Cafer (lightweight, reusable) ---
BASIC_PROP_DATA = {
    'W': {'Density [g/cm^3]': 19.25, 'Valence Electrons': 6},
    'Mo': {'Density [g/cm^3]': 10.28, 'Valence Electrons': 6},
    'Ta': {'Density [g/cm^3]': 16.69, 'Valence Electrons': 5},
    'Nb': {'Density [g/cm^3]': 8.57, 'Valence Electrons': 5},
    'V': {'Density [g/cm^3]': 6.11, 'Valence Electrons': 5},
    'Al': {'Density [g/cm^3]': 2.7, 'Valence Electrons': 3},
    'Ti': {'Density [g/cm^3]': 4.506, 'Valence Electrons': 4},
    'Zr': {'Density [g/cm^3]': 6.52, 'Valence Electrons': 4},
    'Hf': {'Density [g/cm^3]': 13.31, 'Valence Electrons': 4},
    'Cr': {'Density [g/cm^3]': 7.15, 'Valence Electrons': 6},
    'Re': {'Density [g/cm^3]': 21.02, 'Valence Electrons': 7},
    'Ru': {'Density [g/cm^3]': 12.37, 'Valence Electrons': 8},
    'Fe': {'Density [g/cm^3]': 7.874, 'Valence Electrons': 8},
    'Ni': {'Density [g/cm^3]': 8.908, 'Valence Electrons': 10},
    'Co': {'Density [g/cm^3]': 8.9, 'Valence Electrons': 9},
    'Mn': {'Density [g/cm^3]': 7.47, 'Valence Electrons': 7},
    'Cu': {'Density [g/cm^3]': 8.96, 'Valence Electrons': 11},
    'Si': {'Density [g/cm^3]': 2.33, 'Valence Electrons': 4},
    'B': {'Density [g/cm^3]': 2.34, 'Valence Electrons': 3},
    'C': {'Density [g/cm^3]': 2.267, 'Valence Electrons': 4},
    'N': {'Density [g/cm^3]': 1.026, 'Valence Electrons': 5},
    'Y': {'Density [g/cm^3]': 4.47, 'Valence Electrons': 3},
}
...
BASIC_ELAST_DATA = {
    ...
    'Al': {'C11': 38.7, 'C12': 79.2, 'C44': 33, 'B*': 76, 'G*': 26, 'V*': 0.35},
    'Ti': {'C11': 95.9, 'C12': 115.9, 'C44': 40.3, 'B*': 110, 'G*': 44, 'V*': 0.32},
    'Zr': {'C11': 81.8, 'C12': 94.3, 'C44': 30.2, 'B*': 91.1, 'G*': 33, 'V*': 0.34},
    'Hf': {'C11': 73.7, 'C12': 117, 'C44': 51.7, 'B*': 110, 'G*': 30, 'V*': 0.37},
    'Cr': {'C11': 247.6, 'C12': 73.4, 'C44': 48.3, 'B*': 160, 'G*': 115, 'V*': 0.21},
    'Re': {'C11': 325, 'C12': 380.2, 'C44': 158.6, 'B*': 324, 'G*': 185, 'V*': 0.3},
    'Ru': {'C11': 46.6, 'C12': 401.1, 'C44': 173.4, 'B*': None, 'G*': None, 'V*': None},
    'Fe': {'C11': 279.2, 'C12': 148.8, 'C44': 93, 'B*': 170, 'G*': 82, 'V*': 0.291},
    'Ni': {'C11': 148.6, 'C12': 214.3, 'C44': 151.7, 'B*': 181, 'G*': 79, 'V*': 0.31},
    'Co': {'C11': 129.3, 'C12': 140.9, 'C44': 93.5, 'B*': 193, 'G*': 74, 'V*': 0.32},
    'Mn': {'C11': 256.9, 'C12': 272.2, 'C44': 105.4, 'B*': 92.6, 'G*': 76.4, 'V*': 0.35},
    'Cu': {'C11': 129.3, 'C12': 140.9, 'C44': 93.5, 'B*': 137.5, 'G*': 46.5, 'V*': 0.34},
}


def compute_basic_features(df, elements, log_fn=None):
    out = df.copy()
    density = []
    vec = []
    pugh = []
    ...
    for _, row in out.iterrows():
        comp = {el: float(row.get(el, 0.0)) for el in elements}
        active = [el for el, v in comp.items() if v > 0]
        missing = [el for el in active if el not in BASIC_PROP_DATA or el not in BASIC_ELAST_DATA]
        if missing:
            if log_fn:
                log_fn(f"Missing basic data for elements {missing}; setting basic features to NaN.")
            density.append(np.nan)
            vec.append(np.nan)
            pugh.append(np.nan)
            cauchy.append(np.nan)
            continue

        comp_vec = np.array([comp[el] for el in active])
        el_density = np.array([BASIC_PROP_DATA[el]['Density [g/cm^3]'] for el in active])
        el_vec = np.array([BASIC_PROP_DATA[el]['Valence Electrons'] for el in active])
        el_c11 = np.array([BASIC_ELAST_DATA[el]['C11'] for el in active])
        el_c12 = np.array([BASIC_ELAST_DATA[el]['C12'] for el in active])
        el_c44 = np.array([BASIC_ELAST_DATA[el]['C44'] for el in active])
        el_br = np.array([BASIC_ELAST_DATA[el]['B*'] if BASIC_ELAST_DATA[el]['B*'] is not None else np.nan for el in active])
        el_gr = np.array([BASIC_ELAST_DATA[el]['G*'] if BASIC_ELAST_DATA[el]['G*'] is not None else np.nan for el in active])
        el_vr = np.array([BASIC_ELAST_DATA[el]['V*'] if BASIC_ELAST_DATA[el]['V*'] is not None else np.nan for el in active])

        density.append(comp_vec.dot(el_density))
        vec.append(comp_vec.dot(el_vec))

        c11 = comp_vec.dot(el_c11)
        c12 = comp_vec.dot(el_c12)
        c44 = comp_vec.dot(el_c44)
        cauchy.append(c12 - c44)

        if np.any(np.isnan(el_br)) or np.any(np.isnan(el_gr)) or np.any(np.isnan(el_vr)):
            pugh.append(np.nan)
        else:
            b_avgr = comp_vec.dot(el_br)
            g_avgr = comp_vec.dot(el_gr)
            v_avgr = comp_vec.dot(el_vr)
            e_avgr3 = 3 * b_avgr * (1 - 2 * v_avgr)
            g_avgr3 = 0.5 * e_avgr3 / (1 + v_avgr)
            pugh.append(b_avgr / g_avgr3)

    out['Density_g_cc_ROM'] = density
    out['VEC'] = vec
    out['Pugh_Ratio'] = pugh
    out['Cauchy_Pressure'] = cauchy
    return out


def predict_ys_1300c(df, elements, log_fn=None):
    ys = []
    for _, row in df.iterrows():
        comp_full = np.array([float(row.get(el, 0.0)) for el in elements])
        active = [el for el, v in zip(elements, comp_full) if v > 0]
        if not active:
            ys.append(np.nan)
            continue
        comp_active = np.array([float(row.get(el, 0.0)) for el in active])
        try:
            _, _, tau = strength_model.model_Control(active, comp_active, T=1300 + 273.15)
            ys.append(3000 * tau)
        except Exception as exc:
            if log_fn:
                log_fn(f"YS_1300C failed for elements {active}: {exc}")
            ys.append(np.nan)
    df = df.copy()
    df['YS_1300C_MPa'] = ys
    return df
...
if __name__ == '__main__':
    ...
    ##########################################################################
    results_df = pd.read_parquet('SPACE_v0.par')
    
    results_df['Test temperature'] = 1300 #C
