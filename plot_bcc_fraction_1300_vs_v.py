...
def main() -> None:
    ...
    bcc_cols = [col for col in ["EQ 1300C BCC_B2 MOL", "EQ 1300C BCC_B2#2 MOL"] if col in df.columns]
    if not bcc_cols:
        raise SystemExit("Missing BCC columns at 1300C")
    ...
    ax.set_ylabel("BCC fraction (1300C)")
    ax.set_ylim(0, 1.05)
    plt.savefig("bcc_fraction_1300C_vs_v.pdf", bbox_inches="tight")
    plt.close(fig)
